...
int main()
{
	...
	int N=REPETITIONS;
	static FILE * logFile;
	static const char * logFileName = "<testfunctionname>.log";;
	...
	printf("##############################################\n");
	printf("Testing <directive>\n\n");

	fprintf(logFile,"######## OpenMP Validation Suite V 0.94 ######\n");
	fprintf(logFile,"## Repetitions: %3d                       ####\n",N);
	fprintf(logFile,"## Loop Count : %6d                    ####\n",LOOPCOUNT);
	fprintf(logFile,"##############################################\n");
	fprintf(logFile,"Testing <directive>\n\n");

	for(i=0;i<N;i++){
		if(<testfunctionname>(logFile)){
    ...
    if(failed==0){
		...
		fprintf(logFile,"Directive failed the test %i times out of %i. %i were successful\n",failed,N,success);
