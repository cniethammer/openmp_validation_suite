...
!$omp parallel
!$omp do schedule(dynamic, 1) reduction (-: diff)
        do i =1, LOOPCOUNT
          diff = diff - i
        end do
!$omp end do
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(+:dsum)
...
!$omp end parallel
	...
	if(dsum .ne. dknown_sum .and. 
     ...
     &       dsum - dknown_sum,")"
        ...
        ddiff = ( 1-dpt)/(1-dt)
!$omp parallel
!$omp do schedule(dynamic,1) reduction(-:ddiff)
        do i=0, DOUBLE_DIGITS-1
          ddiff = ddiff - dt**i
        end do
!$omp end do
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(*:product)
        do i=1,MAX_FACTOR
           product = product * i
        end do
!$omp end do
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(.and.:logic_and)
        do i=1,LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(.or.:logic_or)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(.eqv.:logic_eqv)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(.neqv.:logic_neqv)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(.or.:logic_neqv)
        do i=1,LOOPCOUNT
           logic_neqv = logic_neqv .or. logics(i)
        end do
!$omp end do
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(iand:bit_and)
        do i=1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(ior:bit_or)
        do i=1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(ieor:exclusiv_bit_or)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(min:min_value)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(max:max_value)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(min:dmin)
        do i = 1, LOOPCOUNT
...
!$omp parallel
!$omp do schedule(dynamic,1) reduction(max:dmax)
        do i = 1, LOOPCOUNT
